...
<div class="table-wrap"><table class="inventory-table"><thead><tr><th>Sản phẩm</th><th>Danh mục</th><th>Giá nhập</th><th>Giá bán</th><th>Giá gốc</th><th>Tồn hiện tại</th><th>Trạng thái</th><th>Tồn tối thiểu</th><th>Tồn tối đa</th><th>Bảo hành</th><th></th></tr></thead><tbody>
<?php foreach($products as $product): $isLow=(int)$product['min_stock']>0 && (int)$product['stock']<=(int)$product['min_stock']; $formId='inventory-'.$product['id']; ?>
  <tr class="<?= $isLow?'low':'' ?>">
    ...
    <td class="num"><input form="<?= $formId ?>" name="cost_price" inputmode="numeric" pattern="[0-9]*" value="<?= (int)$product['cost_price'] ?>"></td>
    <td class="num"><input form="<?= $formId ?>" name="price" inputmode="numeric" pattern="[0-9]*" value="<?= (int)$product['price'] ?>"></td>
    <td class="num"><input form="<?= $formId ?>" name="original_price" inputmode="numeric" pattern="[0-9]*" value="<?= (int)$product['original_price'] ?>"></td>
    <td class="num"><input form="<?= $formId ?>" name="stock" inputmode="numeric" pattern="[0-9]*" min="0" max="1000" value="<?= (int)$product['stock'] ?>"></td>
    <td><?php if($isLow): ?><span class="stock-flag low">Cảnh báo</span><?php else: ?><span class="stock-flag">Bình thường</span><?php endif; ?></td>
    <td class="num"><input form="<?= $formId ?>" name="min_stock" inputmode="numeric" pattern="[0-9]*" min="0" max="1000" value="<?= (int)$product['min_stock'] ?>"></td>
    <td class="num"><input form="<?= $formId ?>" name="max_stock" inputmode="numeric" pattern="[0-9]*" min="0" max="1000" value="<?= (int)$product['max_stock'] ?>"></td>
    <td class="num"><input form="<?= $formId ?>" name="warranty_months" inputmode="numeric" pattern="[0-9]*" min="0" max="999" value="<?= (int)$product['warranty_months'] ?>"></td>
    <td><button form="<?= $formId ?>" class="save" type="submit">Lưu</button></td>
  </tr>
<?php endforeach; if(!$products): ?><tr><td colspan="11" style="text-align:center;padding:30px;color:#718096">Không tìm thấy sản phẩm phù hợp.</td></tr><?php endif; ?>
</tbody></table></div>
