...
function rbacCustomRoleCapabilities(int $userId): array {
    $rows = dbAll(
        "SELECT sr.permissions FROM staff_role_assignments assignment
         INNER JOIN staff_roles sr ON sr.id=assignment.role_id
         LEFT JOIN rbac_staff_role_links link ON link.staff_role_id=sr.id
         WHERE assignment.user_id=? AND link.staff_role_id IS NULL",
        [$userId]
    );
    $capabilities = [];
    foreach ($rows as $row) {
        ...
        foreach (is_array($permissions) ? $permissions : [] as $permission) {
            if (str_starts_with((string)$permission, 'rbac:')) $capabilities[] = substr((string)$permission, 5);
        }
    }
...
function rbacCan(int $userId, string $capability): bool {
    if (in_array($capability, rbacCustomRoleCapabilities($userId), true)) return true;
...
function rbacNormalizeStaffRolePermissions(array $requested): array {
    $legacyPermissions = [
        'orders','products','reviews','contacts','users','vouchers','content','reports',
        'create_order','returns','brands','brand_models','categories','static_pages','stores',
        'chat','promotions','tax_config','staff'
    ];
    $validCapabilities = array_column(dbAll('SELECT DISTINCT capability FROM rbac_capability_rules'), 'capability');
    $normalized = [];
    foreach ($requested as $permission) {
        $permission = trim((string)$permission);
        if (in_array($permission, $legacyPermissions, true)) $normalized[] = $permission;
        if (str_starts_with($permission, 'rbac:') && in_array(substr($permission, 5), $validCapabilities, true)) $normalized[] = $permission;
    }
...
function rbacUsesDetailedMode(int $userId): bool {
    ...
    if (rbacCustomRoleCapabilities($userId)) return true;
