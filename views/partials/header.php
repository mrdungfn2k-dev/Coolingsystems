...
<header class="main">
  <div class="wrap">
    ...
    function setActiveGarageCar(carId){
      var fd=new FormData(); fd.append('_csrf','<?= csrfToken() ?>'); fd.append('car_id', carId);
      fetch('/api/set-active-garage-car', {method:'POST', body:fd}).then(function(r){return r.json();}).then(function(res){ if(res.ok) location.reload(); });
    }
    document.addEventListener('click', function(e){ var w=document.querySelector('.quick-car-select-wrap'); if(w && !w.contains(e.target)){ var m=document.getElementById('quickCarDropdownMenu'); if(m) m.style.display='none'; } });
    </script>
    <div class="hotline">
