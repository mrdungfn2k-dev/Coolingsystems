...
<div class="table-wrap"><table class="inventory-table"><thead><tr><th>Sản phẩm</th><th>Danh mục</th><th>Vị trí kho</th><?php if($canViewCost): ?><th>Giá nhập</th><?php endif; ?><th>Giá bán</th><th>Giá buôn Gara</th><th>Giá gốc</th><th>Tồn hiện tại</th><th>Trạng thái</th><th>Tồn tối thiểu</th><th>Tồn tối đa</th><th>Bảo hành</th><th></th></tr></thead><tbody>
<?php foreach($products as $product): $isLow=(int)$product['min_stock']>0 && (int)$product['stock']<=(int)$product['min_stock']; $formId='inventory-'.$product['id']; ?>
  <tr class="<?= $isLow?'low':'' ?>">
    ...
    <td><?php if(!empty($product['location_code'])): ?><a href="/admin/locations" style="text-decoration:none" title="Bấm để chuyển tới Sơ đồ vị trí kho"><span style="background:#e0f2fe;color:#0369a1;border:1px solid #bae6fd;padding:3px 8px;border-radius:4px;font-family:monospace;font-weight:700;font-size:11px"><?= e($product['location_code']) ?></span></a><?php else: ?><a href="/admin/locations" style="color:#94a3b8;font-size:11px;text-decoration:none" title="Chưa gán vị trí kho — Bấm để chuyển tới Sơ đồ vị trí kho">+ Gán vị trí</a><?php endif; ?></td>
<?php if($canViewCost): ?>    <td class="num"><input form="<?= $formId ?>" name="cost_price" inputmode="numeric" pattern="[0-9]*" <?= $canEditCost ? '' : 'readonly' ?> value="<?= (int)$product['cost_price'] ?>"></td><?php else: ?>    <td class="num"><input form="<?= $formId ?>" type="hidden" name="cost_price" value="<?= (int)$product['cost_price'] ?>"><span>--</span></td><?php endif; ?>
    ...
    <td class="num"><input form="<?= $formId ?>" name="wholesale_price" inputmode="numeric" pattern="[0-9]*" <?= $canEditPrice ? '' : 'readonly' ?> value="<?= (int)($product['wholesale_price'] ?? round($product['price'] * 0.85)) ?>"></td>
...
<?php if($canSaveInventory): ?>    <td><button form="<?= $formId ?>" class="save" type="submit">Lưu</button></td><?php endif; ?>    <td><a href="/admin/inventory/<?= (int)$product['id'] ?>/ledger" style="display:inline-block;padding:6px 12px;font-size:11px;color:#1a3258;border:1px solid #1a3258;border-radius:4px;text-decoration:none;font-weight:700;background:#fff;white-space:nowrap" title="Xem thẻ kho">Thẻ kho</a></td>
  </tr>
<?php endforeach; if(!$products): ?><tr><td colspan="<?= $canViewCost ? 14 : 13 ?>" style="text-align:center;padding:30px;color:#718096">Không tìm thấy sản phẩm phù hợp.</td></tr><?php endif; ?>
</tbody></table></div>
