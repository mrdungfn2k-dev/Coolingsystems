...
<section class="block"><div class="wrap">
  ...
  <?php else:
    ...
    $shipping = $total >= $freeShipThreshold ? 0 : $shipFee;
    $discount = 0;
    $qtyDiscount = 0;
  ...
  <div class="cart-wrap">
    ...
    <div class="cart-summary">
      ...
      <div class="sum-row"><span>Phí vận chuyển</span><span><?=$shipping===0?'<span style="color:#27ae60">Miễn phí</span>':vnd($shipping)?></span></div>
      ...
      <div class="cart-notice"> Miễn phí ship từ 2.000.000 ₫<br> Bảo hành chính hãng<br> Đổi trả 7 ngày</div>
      ...
      <div style="background:#fee2e2;color:#dc2626;padding:12px;border-radius:8px;font-size:13px;font-weight:600;margin-top:16px;text-align:center">
         Giỏ hàng có <?= count($items) ?> sản phẩm. Tối đa 5 sản phẩm mỗi đơn hàng.<br>Vui lòng bớt sản phẩm để tiếp tục.
      </div>
      <button disabled class="btn btn-block btn-lg" style="margin-top:10px;display:block;text-align:center;border-radius:6px;background:#ccc;color:#888;cursor:not-allowed;border:none;height:48px">Thanh toán →</button>
      <?php else: ?>
      <a href="/customer/checkout" class="btn btn-gold btn-block btn-lg" style="margin-top:16px;display:block;text-align:center;border-radius:6px">Thanh toán →</a>
      <?php endif; ?>
    </div>
  </div>
  <?php endif;?>
